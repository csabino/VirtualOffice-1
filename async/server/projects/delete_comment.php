...
if (isset($_SERVER['HTTP_X_REQUESTED_WITH']) && $_SERVER['HTTP_X_REQUESTED_WITH']=='XMLHttpRequest')
  {
    ...
    $dataArray = array("project_update_id"=>$project_update_id, "user_id"=>$user_id, "comment_id"=>$comment_id);

    // check that fields are not blank
    if ($comment_id!='' && $user_id!='' && $project_update_id!='')
    {
        $project = new Project();
        $comment = $project->delete_project_update_comment($dataArray);
